...
public class removeCycle {
    ...
    public static void removecy() {
        ...
        if (cycle) {
            ...
            while (slow != fast) {
                prev = fast; // Last Node stored by this step
                slow = slow.next; // +1
                fast = fast.next; // +1
            }

            // Remove the cycle by setting prev.next to null
            prev.next = null;
        }
    }

    public static void main(String[] args) {// find the case to check removecy()fn
        LLMethods linkedList = new LLMethods();
        ...
        linkedList.head.next.next.next = linkedList.head; // Create a cycle

        boolean hasCycleBeforeRemoval = isCycle();
        System.out.println("Does the linked list have a cycle before removal? " + hasCycleBeforeRemoval);

        removecy(); // Remove the cycle

        boolean hasCycleAfterRemoval = isCycle();
        System.out.println("Does the linked list have a cycle after removal? " + hasCycleAfterRemoval);
    }
